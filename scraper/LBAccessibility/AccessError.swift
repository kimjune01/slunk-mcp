...
public struct AccessError: Error, Identifier {
    ...
    /// Initializes from a wrapped legacy accessibility error value.
    /// - Parameter value: wrapped Legacy accessibility error value.
    @Accessibility init?(axValue value: CFTypeRef) {
...
extension AccessError: Legacy {
    var axValue: CFTypeRef {
        guard var error = AXError(rawValue: rawValue) else {
            // TODO: Add logging in production.
            assertionFailure("Failed to convert error value \(rawValue) to an AXError value")
            return kCFNull
        }
